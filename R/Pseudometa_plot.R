Pseudometa_plot <- function(PseudometaObj, plot = "Gene_Volcano", method = "Spearman", 
                            title = NULL, title_size = 14, x_axis_text_size = 12, y_axis_text_size = 12, 
                            x_axis_title_size = 14, y_axis_title_size = 14, 
                            legend_title_size = 12, legend_text_size = 10, 
                            legend_frame_size = 0, point_size = 1.5) {
  ...
  if (plot == "method_cor") {
    ...
    plot_title <- ifelse(is.null(title), paste("Gene Plot of", method, "Method"), title)
    ...
    volcano_plot <- ggplot2::ggplot(results_df, ggplot2::aes(x = x_position, y = p.value, color = monotony)) +
      ...
      ggplot2::theme(
        ...
        legend.key = ggplot2::element_rect(size = legend_frame_size)
      )
